# Weather Summoning

Weather Summoning is an OSRIC Chapter II spell available as: Druid 6.

## Spell Entries

### Druid 6

**School:** Druidic Conjuration/Summoning

| Field | OSRIC Value |
| --- | --- |
| Class/List | Druid 6 |
| Range | 0 |
| Duration | See below |
| Area of Effect | See below |
| Components | V,S,M |
| Casting Time | 1 turn |
| Saving Throw | None |

### Spell Description

The area of effect of this spell is d% square miles, but except as noted above, the spell is similar to the clerical spell control weather, allowing the druid to affect, but not direct, existing weather conditions. Unlike clerical and druidic spells, magic user spells (also known as Arcane spells in the OSRIC system) do not require a holy symbol or mistletoe as material components.

## OSRIC Source

Verified against OSRIC core rules, Chapter II, Spells. No DRG 1e table rule is added here.

## Spell Navigation

- **[← Previous Spell: Water Breathing](/1e/spells/water-breathing/)**
- **[All Spells](/1e/spells/all-spells/)**
- **[Next Spell: Web →](/1e/spells/web/)**
