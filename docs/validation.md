...
cd frontend
npm install
VITE_API_BASE_URL=http://127.0.0.1:8010/api npm run build
```

Expected result: Vite finishes with a production bundle in `frontend/dist`.

Restart `discord-bot.service` and `russo-backend.service`, then run `/ledger show` again. The character should still load from PostgreSQL.
