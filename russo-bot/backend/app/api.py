...
def list_vault_players(db: Session = Depends(get_db)) -> list[dict]:
...
def create_vault_player(data: dict, db: Session = Depends(get_db)) -> dict:
...
def get_vault_player(user_id: int, db: Session = Depends(get_db)) -> dict:
    return player_payload(get_player_or_404(db, user_id))


@router.put("/1e/players/{user_id}")
def update_vault_player(user_id: int, data: dict, db: Session = Depends(get_db)) -> dict:
...
def create_vault_equipment(data: dict, db: Session = Depends(get_db)) -> dict:
...
def update_vault_equipment(equipment_id: int, data: dict, db: Session = Depends(get_db)) -> dict:
...
def delete_vault_equipment(equipment_id: int, db: Session = Depends(get_db)) -> dict:
    item = db.get(EquipmentCatalog, equipment_id)
    if item is None:
        raise HTTPException(status_code=404, detail="Equipment not found.")
    item.archived = True
...
def list_vault_campaigns(include_archived: bool = False, db: Session = Depends(get_db)) -> list[dict]:
...
def create_vault_campaign(data: dict, db: Session = Depends(get_db)) -> dict:
    campaign = Campaign(
        name=data["name"],
...
def get_vault_campaign(campaign_id: int, db: Session = Depends(get_db)) -> dict:
...
def update_vault_campaign(campaign_id: int, data: dict, db: Session = Depends(get_db)) -> dict:
    campaign = db.get(Campaign, campaign_id)
    if campaign is None:
        raise HTTPException(status_code=404, detail="Campaign not found.")
...
def delete_vault_campaign(campaign_id: int, db: Session = Depends(get_db)) -> dict:
    campaign = get_campaign_or_404(db, campaign_id)
...
def add_vault_campaign_player(campaign_id: int, data: dict, db: Session = Depends(get_db)) -> dict:
    campaign = get_campaign_or_404(db, campaign_id)
...
def remove_vault_campaign_player(campaign_id: int, user_id: int, db: Session = Depends(get_db)) -> dict:
...
def assign_vault_character_to_campaign(campaign_id: int, character_id: int, db: Session = Depends(get_db)) -> dict:
    campaign = get_campaign_or_404(db, campaign_id)
...
def unassign_vault_character_from_campaign(campaign_id: int, character_id: int, db: Session = Depends(get_db)) -> dict:
    character = get_vault_character_or_404(db, character_id)
...
def list_vault_safe_storage(campaign_id: int, db: Session = Depends(get_db)) -> list[dict]:
...
def create_vault_safe_storage(campaign_id: int, data: dict, db: Session = Depends(get_db)) -> dict:
    campaign = get_campaign_or_404(db, campaign_id)
    name = data.get("name")
    if not name:
...
def update_vault_safe_storage(campaign_id: int, location_id: int, data: dict, db: Session = Depends(get_db)) -> dict:
...
def archive_vault_safe_storage(campaign_id: int, location_id: int, db: Session = Depends(get_db)) -> dict:
    location = db.get(SafeStorageLocation, location_id)
    if location is None or location.campaign_id != campaign_id:
        raise HTTPException(status_code=404, detail="Safe storage location not found.")
...
def list_vault_characters(user_id: Optional[int] = None, include_archived: bool = False, db: Session = Depends(get_db)) -> list[dict]:
...
def create_vault_character(data: dict, db: Session = Depends(get_db)) -> dict:
    ensure_vault_seeded(db)
...
def get_vault_character(character_id: int, db: Session = Depends(get_db)) -> dict:
    return character_payload(get_vault_character_or_404(db, character_id))


@router.put("/1e/characters/{character_id}")
def update_vault_character(character_id: int, data: dict, db: Session = Depends(get_db)) -> dict:
    character = get_vault_character_or_404(db, character_id)
    validate_character_choice(data)
...
def delete_vault_character(character_id: int, db: Session = Depends(get_db)) -> dict:
    character = get_vault_character_or_404(db, character_id)
...
def add_vault_inventory(character_id: int, data: dict, db: Session = Depends(get_db)) -> dict:
    character = get_vault_character_or_404(db, character_id)
...
def update_vault_inventory(character_id: int, inventory_id: int, data: dict, db: Session = Depends(get_db)) -> dict:
    character = get_vault_character_or_404(db, character_id)
...
def delete_vault_inventory(character_id: int, inventory_id: int, db: Session = Depends(get_db)) -> dict:
    character = get_vault_character_or_404(db, character_id)
    item = db.get(CharacterInventory, inventory_id)
    if item is None or item.character_id != character.id:
        raise HTTPException(status_code=404, detail="Inventory item not found.")
...
def add_vault_character_spell(character_id: int, data: dict, db: Session = Depends(get_db)) -> dict:
    character = get_vault_character_or_404(db, character_id)
...
def update_vault_character_spell(character_id: int, character_spell_id: int, data: dict, db: Session = Depends(get_db)) -> dict:
    character = get_vault_character_or_404(db, character_id)
...
def add_vault_weapon_proficiency(character_id: int, data: dict, db: Session = Depends(get_db)) -> dict:
    character = get_vault_character_or_404(db, character_id)
    equipment = db.get(EquipmentCatalog, int(data["equipment_id"]))
